...
import { Table, Tag, DatePicker, Select, Empty, Modal, Spin, message } from "antd";
import { fetchLaunches, fetchLaunchpads, fetchRockets, fetchPayloads } from "../api/spacex";
...
const Dashboard = () => {
  ...
  setLoading(true);
  try {
    const [launchRes, launchpadsRes, rocketsRes, payloadsRes] = await Promise.all([
      fetchLaunches(),
      fetchLaunchpads(),
      fetchRockets(),
      fetchPayloads(),
  ...
  useEffect(() => {
    ...
    if (selectedStatus !== "All Launches") {
      result = result.filter((item) => item.status.toLowerCase() === selectedStatus.toLowerCase());
    }
  ...
  const columns = [
    {
      ...
      dataIndex: "no",
      align: "center",
      onHeaderCell: () => ({ style: { backgroundColor: "#F4F5F7" } }),
    },
    {
      title: "Launched (UTC)",
    ...
    },
    {
      ...
      dataIndex: "status",
      align: "center",
      render: (status) => <Tag color={statusColors[status.toLowerCase()]}>{status}</Tag>,
      onHeaderCell: () => ({ style: { backgroundColor: "#F4F5F7" } }),
    },
    {
      title: "Rocket",
  ...
  return (
    <div className="p-4 min-h-screen  flex flex-col items-center">
      <div className="bg-white flex flex-col mb-4 shadow-md w-full justify-center items-center p-2">
        <img src="/spacex_logo.png" alt="SpaceX Logo" className="w-40 md:w-60 mb-2 object-contain" />
      </div>
        ...
        {modalData && (
          <div className="flex flex-col gap-4">
            <div className="flex flex-col sm:flex-row items-center gap-4">
              <img
                src={modalData.links?.patch?.small || "https://upload.wikimedia.org/wikipedia/commons/thumb/2/2f/SpaceX_CRS-1_Patch.png/480px-SpaceX_CRS-1_Patch.png"}
                alt="Mission Patch"
              ...
              <div className="flex flex-col text-center sm:text-left">
                ...
                <Tag color={statusColors[modalData.status.toLowerCase()]} className="w-fit mx-auto sm:mx-0 mt-1">
                  {modalData.status}
                </Tag>
              </div>
            </div>
            ...
            <div className="flex flex-col border rounded-md overflow-hidden">
              {[{ label: "Flight Number", value: modalData.no },
                ...
                { label: "Launch Date", value: moment(modalData.launchDate).format("DD MMM YYYY HH:mm") },
                ...
                <div
                  key={idx}
                  className={`flex justify-between px-4 py-2 text-sm ${idx !== 0 ? "border-t" : ""}`}
                >
